...
class Alternative_Sampler1_FastGCN(Sampler):
    def __init__(self, pre_probs, features, adj, **kwargs):
        super().__init__(features, adj, **kwargs)
        # NOTE: uniform sampling can also has the same performance!!!!
        # try, with the change: col_norm = np.ones(features.shape[0])
...
class Alternative_Sampler2_FastGCN(Sampler):
    def __init__(self, pre_probs, features, adj, **kwargs):
        super().__init__(features, adj, **kwargs)
        # NOTE: uniform sampling can also has the same performance!!!!
        # try, with the change: col_norm = np.ones(features.shape[0])
        ...
        eigen_vals,eigen_vecs = sparse_eigsh(adj)
        i = np.argmin((eigen_vals - np.ones_like(eigen_vals))**2)
        v = eigen_vecs[:,i]
        if (v.sum() < 0):
            v = -v
        v[v<0] = 0
